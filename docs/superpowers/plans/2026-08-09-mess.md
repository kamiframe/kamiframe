...
- **Sickness and death.** Mess feeds them; that is the next plan.
- **Care variations.** The clean action added here takes no variation
  argument yet — the variations plan adds one to all four actions at once.
- **Any screen change.** Nothing draws poops, flies or stink lines yet. The
  simulation gains mess; the art and the drawing come later.
- **The Lua API.** `kf_pet_clean()` is not exposed to scripts here.
