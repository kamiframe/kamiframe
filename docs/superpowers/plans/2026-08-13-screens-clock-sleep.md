...
**Task 7 (sleep on screen) has now landed too** — see ADR 0049.
`pet.asleep()`/`pet.wake()` bound to Lua; a new, generically useful
`kf.button(name)` binding; the wander freeze while asleep in
`kf_creature_presenter.cpp`, proven by a check that runs 300 real frames
and asserts the creature's position does not move (exactly the assertion
this plan named as missing); the tuck-in interaction (B, while drowsy) and
its `futon_idle_s` scenery — one generic sprite for every stage, authorized
and generated by Chris mid-task, superseding this task's own original
"draw it as boxes" instruction (see ADR 0049's "What was built and
discarded"); waking deliberately costs 5% happiness
(`wake_happiness_cost_mp`). 48/48 tests green (47 plus
`sleep_screen_check`). **Still needs Chris at the board** — nothing about
sleep has run on hardware, and the drowsy hour, wiggle amplitude and ZZZ
...
| There is no Lua time binding | read all three `luaL_Reg` tables | `kf` = `log`, `report`, `home_screen_active` (`sdk/lua/kf_lua_port.cpp:112`) plus ten drawing/screen entries including **`screen`, added by Task 1** (`kf_lua_scene.cpp:861`); `pet` = **23**, `pet.stage_seconds()` added since (`:366`); `creature` = 5 read-only (`:447`). **Nothing exposes a clock still** — **stale as of Task 4 (added `kf.time/hour/minute/clock_set/set_clock`) and Task 7 (added `kf.button`; added `pet.wake`/`pet.asleep`, `pet` now 25)** |
...
### Task 7: Sleep on screen — LANDED (2026-08-11), ADR 0049

The game's half. Uses the 18 single-frame sleeping sprites that already ship.

**Status:** Landed, all requirements below satisfied. `pet.asleep()`/
`pet.wake()` bound to Lua; a new `kf.button(name)` binding (generically
useful beyond sleep, not only for this task); the wander freezes in
`kf_creature_presenter.cpp` while asleep, proven by a headless check that
runs 300 real frames and asserts position does not move — exactly the
assertion this section originally asked for and named as missing; the
sleeping pose shows automatically via the existing `kf_creature_pose_for()`
precedence, no script change needed for that part. 48/48 tests green (47
plus `sleep_screen_check`). See ADR 0049 for the full account, including
two of this task's own new test assertions caught being vacuous on first
write and rewritten.

**One requirement below was superseded mid-task by an owner decision, not
by this task's own judgement**: bedding is real art (`futon_idle_s`, one
generic sprite for every stage), not scene boxes — see ADR 0049's "What
was built and discarded" for the box version that was built, tested, and
replaced. The requirement list is left as originally written below, with
each line marked against what actually landed, per this plan's own rule
...
- [x] `creature.lua` shows the sleeping pose, the creature settled where it
      ...
      the obvious bug and the parity check will not catch it. **Landed**: the
      freeze is in `kf_creature_presenter.cpp` (simulator layer, not
...
- [x] Bedding: put away by the creature itself in the morning, per the spec.
      ...
      generic `futon_idle_s` sprite (48×48, every stage, no per-stage/
      per-direction variants, looked up by literal name like
      `shrine_idle_s`) partway through this task. A boxes-only version was
      built and verified first; see ADR 0049 for both the final design and
      the discarded one. Waking puts it away: `creature.lua` resets
      `tucked_in` on the `was_asleep && !asleep` edge, proven by a headless
      assertion that samples the vacated pixel at the original bed position
      (the first version of this assertion was vacuous — see ADR 0049's
      non-vacuity table).
- [x] Waking it deliberately is allowed and costs happiness. Small.
      **Landed**: `kf_pet_wake()` (`hakoniwaos/`), 5% happiness
      (`wake_happiness_cost_mp`, a new `kf_pet_config` field, illustrative
      like every other figure in `kf_pet_default_config()`), wired to the A
      button while asleep via the new `kf.button()` binding and `pet.wake()`.
- [x] **A sleeping creature is a static frame** — the sleeping sprites are
      single-frame (finding 1). Noted for Chris as an art decision, with the
      cost stated plainly: a breathing loop is 6 stages x 3 directions x N
