...
typedef enum {
     ...
     * the caller reads ::format if it cares. Since Task 2, kf/blit.h's
     * kf_blit_frame()/kf_blit_frame_mirrored() branch on ::format and draw
     * either shape correctly -- drawing one no longer panics. See
