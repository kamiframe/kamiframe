 ...
 * pet.neglect_seconds() (the sickness-and-death plan) packed into one
 * integer (flags * 10000000 + neglect_seconds, flags = sick*1 + dead*2 --
 * neglect_seconds is capped at sickness_death_seconds, comfortably under
 * 10000000 for any config this project ships). Touches nothing; run after
 * the session has already been cared for at least once (mess/clean above
 * leaves it that way), then left alone long enough to fall ill, the same
 * "mutate elsewhere, then check the live C++ state independently" proof
 * every other script here uses.
 * kKfLuaPetReactionProofScriptSource Care variations (the care-variations
 * plan). Works out, IN Lua, which variation of feeding this session's own
 * (randomly rolled) pet.base_trait() it dislikes -- via pet.reaction_to(),
 * proving that binding too rather than assuming C++ and Lua agree on it --
 * then feeds with exactly that variation and reports pet.last_reaction()
 * and pet.last_care_action() packed into one integer (reaction * 10 +
 * care_action). Deliberately picks the DISLIKED variation rather than an
 * arbitrary one: the preference table's own exactly-one-of-each invariant
 * (run_pet_preferences_check()) guarantees a disliked variation exists for
 * every possible base_trait, so the reaction is known to land on a specific
 * nonzero value (KF_PET_REACTION_DISLIKED) no matter which trait this
 * session happens to have rolled -- run_lua_pet_check() asserts that
 * directly against the live C++ state before comparing, so this cannot pass
 * on two neutral zeroes the way the mess and health stages above guard
 * against.
 */
